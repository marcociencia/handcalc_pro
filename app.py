...
<style>
    ...
    .manual-display {
        ...
        display: inline-block; white-space: pre; text-align: left;
    ...
    .theory-box {
        background: #f0f4ff; border-left: 5px solid #667eea; border-radius: 10px;
...
class MathSolver:
    ...
    def _solve_linear_blocks(self, eq_str):
        try:
            ...
            if left_expr is None or right_expr is None:
                return [{"type": "html", "content": "<div class='step-box'>❌ Invalid expression.</div>"}]
            ...
            if len(coeffs) == 2:
                ...
                a = b = 0
            if a == 0:
                return [{"type": "html", "content": "<div class='step-box'>⚠️ Not a linear equation (a = 0).</div>"}]
            x_sol = -b / a
            blocks = [
                {"type": "html", "content": """
                ...
                {"type": "latex", "content": f"{latex(expr)} = 0"},
                {"type": "text", "content": f"Identifying coefficients: a = {a}, b = {b}"},
                {"type": "latex", "content": f"x = \\frac{{-{b}}}{{{a}}} = {latex(sp.nsimplify(x_sol))}"},
                {"type": "html", "content": f'<div class="result-box">🎯 <strong>Solution: $x = {latex(sp.nsimplify(x_sol))}$</strong></div>'}
            ]
            return blocks
    ...
    def _solve_quadratic_blocks(self, func_str):
        try:
            ...
            if len(coeffs) == 3:
                ...
                a = coeffs[0]
            if a == 0:
                return [{"type": "html", "content": "<div class='step-box'>⚠️ Not quadratic (a = 0).</div>"}]
            disc = b**2 - 4*a*c
            blocks = [
                {"type": "html", "content": """
                <div class="theory-box">
                ...
                {"type": "latex", "content": f"{latex(expr)} = 0"},
                {"type": "text", "content": f"Discriminant Δ = {disc}"}
            ]
            if disc >= 0:
                sqrt_disc = math.sqrt(disc)
                x1 = (-b + sqrt_disc) / (2*a)
                x2 = (-b - sqrt_disc) / (2*a)
                blocks.append({"type": "latex", "content": f"x_1 = {latex(sp.nsimplify(x1))}, \\quad x_2 = {latex(sp.nsimplify(x2))}"})
                blocks.append({"type": "html", "content": f'<div class="result-box">🎯 <strong>Roots: $x_1 = {latex(sp.nsimplify(x1))},\\; x_2 = {latex(sp.nsimplify(x2))}$</strong></div>'})
            else:
                real_p = -b / (2*a)
                imag_p = math.sqrt(-disc) / (2*a)
                blocks.append({"type": "text", "content": f"Complex roots: x = {real_p:.4f} ± {imag_p:.4f}i"})
                blocks.append({"type": "html", "content": '<div class="result-box">🎯 <strong>Complex roots</strong></div>'})
            return blocks
        except Exception as e:
            return [{"type": "html", "content": f"<div class='step-box'>❌ Error: {str(e)}</div>"}]
    ...
    def _differentiate_blocks(self, func_str, var='x', eval_pt=None):
        try:
            ...
            if expr is None:
                return [{"type": "html", "content": "<div class='step-box'>❌ Invalid function.</div>"}]
            sym_var = Symbol(var)
            ...
            blocks = [
                ...
                {"type": "latex", "content": f"f({var}) = {latex(expr)}"},
                {"type": "latex", "content": f"f'({var}) = {latex(df)}"},
                {"type": "latex", "content": f"f'({var}) = {latex(simplified_df)} \\;\\text{{(simplified)}}"},
            ]
            if eval_pt and str(eval_pt).strip():
                try:
                    pt = parse_expr(str(eval_pt).replace('^', '**'))
                    val = simplified_df.subs(sym_var, pt)
                    blocks.append({"type": "latex", "content": f"f'({latex(pt)}) = {latex(val)}"})
                    if val.is_number and not val.is_Integer:
                        blocks.append({"type": "text", "content": f"≈ {float(val):.4f}"})
                except Exception:
                    blocks.append({"type": "text", "content": "Could not evaluate at that point."})
            blocks.append({"type": "html", "content": f'<div class="result-box">🎯 $$\\boxed{{f\'({var}) = {latex(simplified_df)}}}$$</div>'})
            return blocks
        except Exception as e:
            return [{"type": "html", "content": f"<div class='step-box'>❌ Error: {str(e)}</div>"}]
    ...
    def _integrate_blocks(self, func_str, var='x', lower=None, upper=None):
        try:
            ...
            blocks = [
                ...
                {"type": "latex", "content": f"\\int \\left({latex(expr)}\\right) \\, d{var} = {latex(simplified_prim)} + C"},
            ]
            if lower and upper and str(lower).strip() and str(upper).strip():
                try:
                    ...
                    blocks.append({"type": "latex", "content": f"\\int_{{{latex(a)}}}^{{{latex(b)}}} \\left({latex(expr)}\\right) \\, d{var} = {latex(def_res)}"})
                    blocks.append({"type": "html", "content": f'<div class="result-box">🎯 $$\\boxed{{= {latex(def_res)}}}$$</div>'})
                except Exception:
                    blocks.append({"type": "text", "content": "Invalid limits."})
            else:
                blocks.append({"type": "html", "content": f'<div class="result-box">🎯 $$\\boxed{{\\int \\left({latex(expr)}\\right) \\, d{var} = {latex(simplified_prim)} + C}}$$</div>'})
            return blocks
        except Exception as e:
            return [{"type": "html", "content": f"<div class='step-box'>❌ Error: {str(e)}</div>"}]
